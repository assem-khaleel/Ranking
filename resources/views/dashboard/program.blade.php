...
@section('wrapper')
    ...
    <div class="container-fluid">
        <form action="{{route('home.programDashboard')}}" class="form-horizontal"
              method="get">
            <div class="form-body">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <select class="select2 form-control custom-select"
                                        style="width: 100%; height:36px;" name="program"
                                        data-placeholder="{{trans('result.selectProgram')}}">
                                    <option value="">{{trans('result.selectProgram')}}</option>
                                @foreach($programs as $program)
                                        <option value="{{$program->id}}" {{ !empty($currentProgram) && $currentProgram->id == $program->id ? 'selected' : '' }}>{{$program->name}}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="wrapper">
                    <div class="list">
                        @for($i=date('Y'); $i>=2000; $i--)
                            <a href="{{route('home.programDashboard',['program' => $programId , 'year' => $i])}}"
                                ...
                                <div class="row p-t-10 p-b-10">
                                    ...
                                    <div class="col m-r-40">
                                        <div class="chart easy-pie-chart-1"
                                             data-percent="{{$totalSystem[$rankingSystem->id]}}"><span
                                                    class="percent">{{$totalSystem[$rankingSystem->id]}}</span>
                                ...
                                <div class="row m-t-40">
                                    ...
                                    @for($i = 1; $i <= 12; $i++)
                                        ...
                                        <div class="col-lg-2 col-md-6 m-b-30 text-center show-system" id="show-system"
                                            ...
                                            <div>
                                                <small>{{trans('program.progress')}}
                                                    : {{$results[$rankingSystem->id][$i]}}</small>
                                            </div>
    ...
    @push('script')
        ...
        <script>
            ...
            $(".show-system").click(function (e) {
                ...
                $.ajax({
                    url: '/showProgramDashboard/' + program_id + '/' + system_id + '/' + month + '/' + lang + '/' + year,
                    dataType: 'json',
                    success: function (e) {
